...
static void concatenate() {
    ...
    const int32_t length = a->length + b->length;
...
static double toDouble(const Value v) {
    ...
    if (IS_INTEGER(v)) return (double)AS_INTEGER(v);

    runtimeError("Operand must be a number.");
    return 0; // unreachable if you exit properly
}
...
#define BINARY_OP(op) \
    do { \
        ...
        Value aVal = pop(); \
        \
        if (IS_INTEGER(aVal) && IS_INTEGER(bVal)) { \
            push(INTEGER_VAL(AS_INTEGER(aVal) op AS_INTEGER(bVal))); \
        } else if ((IS_NUMBER(aVal) || IS_INTEGER(aVal)) && \
                   (IS_NUMBER(bVal) || IS_INTEGER(bVal))) { \
            double a = IS_INTEGER(aVal) ? (double)AS_INTEGER(aVal) : AS_NUMBER(aVal); \
            double b = IS_INTEGER(bVal) ? (double)AS_INTEGER(bVal) : AS_NUMBER(bVal); \
...
#define BOOL_OP(op) \
    do { \
        Value b = pop(); \
        Value a = pop(); \
        push(BOOL_VAL(toDouble(a) op toDouble(b))); \
    } while (false)
    ...
    while (true)
    {
        ...
        switch (instruction = READ_BYTE())
        {
            ...
            case OP_ADD: {
                ...
                if (IS_INTEGER(aVal) && IS_INTEGER(bVal)) {
                    push(INTEGER_VAL(AS_INTEGER(aVal) + AS_INTEGER(bVal)));
                }
                else if ((IS_NUMBER(aVal) || IS_INTEGER(aVal)) &&
                        (IS_NUMBER(bVal) || IS_INTEGER(bVal))) {
                    double a = IS_INTEGER(aVal) ? (double)AS_INTEGER(aVal) : AS_NUMBER(aVal);
            ...
            case OP_INT_DIVIDE: BINARY_OP(/); break;
            case OP_DIVIDE: {
                Value bVal = pop();
                Value aVal = pop();

                if ((IS_NUMBER(aVal) || IS_INTEGER(aVal)) &&
                    (IS_NUMBER(bVal) || IS_INTEGER(bVal))) {
                    double a = IS_INTEGER(aVal) ? (double)AS_INTEGER(aVal) : AS_NUMBER(aVal);
                    double b = IS_INTEGER(bVal) ? (double)AS_INTEGER(bVal) : AS_NUMBER(bVal);
            ...
            case OP_GREATER_EQUAL:  BOOL_OP(>=); break;
            case OP_LESS_EQUAL:  BOOL_OP(<=); break;
